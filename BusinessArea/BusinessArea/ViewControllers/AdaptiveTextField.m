...
@implementation AdaptiveTextField
...
- (void)markTextFieldsWithTagInView:(UIView*)view
{
    ...
    if ([self.textFields count] == 0){
        for(UIView *subView in view.subviews){
            if ([subView isKindOfClass:[AdaptiveTextField class]]){
                AdaptiveTextField *textField = (AdaptiveTextField*)subView;
                textField.tag = index;
...
-(void) keyboardDidShow:(NSNotification *) notification
{
    ...
    if (![_textField isKindOfClass:[AdaptiveTextField class]]) return;
...
- (void) nextButtonIsClicked:(id)sender
{
    ...
    AdaptiveTextField *textField =  [self.textFields objectAtIndex:++tagIndex];
...
-(BOOL)checkInputIsEqualToString:(NSString *)string tip:(NSString *)tipTitle
{
    inputCheck_ = [[TextFieldInputCheck alloc] init];
    [self resignFirstResponder];
    NSError *error = nil;
    
    BOOL equal = [inputCheck_ checkTextFieldInputIsEqualToString:self string:string tip:tipTitle error:&error];
    if (!equal) {
