...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletedBlock)completedBlock
{
    ...
    if (url)
        ...
        if (urlString.length != 0) {
            ...
            if ([urlString rangeOfString:@"portrait"].location == NSNotFound && [urlString rangeOfString:@"pet"].location == NSNotFound) {
                activity = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(self.frame.size.width / 2 - activity_width / 2, self.frame.size.height / 2 - activity_height / 2, activity_width, activity_height)];
...
//                                                                         [[NSNotificationCenter defaultCenter] postNotificationName:FinishDownloadImage object:self userInfo:[NSDictionary dictionaryWithObject:image forKey:FinishedImage]];
                                                                 if (activity != nil) {
